foverlaps_count_by_row_limit_with_fallback <- function(
  ...
  dt1_valid <- dt1[dt1[[kol_a]] %in% gemeenschappelijke_groepen]
  dt2_valid <- dt2[dt2[[kol_a]] %in% gemeenschappelijke_groepen]
  
  groepsgroottes <- dt1_valid[, .N, by = kol_a]
  groepsgroottes <- groepsgroottes[groepsgroottes[[kol_a]] %in% gemeenschappelijke_groepen]
  ...
  if (length(batches) > 1) message(sprintf("Aantal batches: %d", length(batches)))
  
  result_valid_list <- vector("list", length(batches))
  
  if (show_progress) pb <- txtProgressBar(min = 0, max = length(batches), style = 3)
  
  for (i in seq_along(batches)) {
    groepen_i <- batches[[i]]
    
    #if (.verbose) message("batch ", i, "/",length(batches))
    
    dt1_b <- dt1_valid[dt1_valid[[kol_a]] %in% groepen_i]
    dt2_b <- dt2_valid[dt2_valid[[kol_a]] %in% groepen_i]
    ...
    tryCatch({
      ...
      # ivm Join overflow:
      setkey(dt1_b, start, end) # intervalkant keyen
      
      # dt2_b kleiner maken
      dt2_u <- dt2_b[, .(n_point = .N), by = c(kol_a, "start", "end")]
      
      # joinen op intervals vs points
      hits <- dt1_b[
        dt2_u,
        on = c(kol_a,
               "start <= start",
      ...
      # terug naar per-rij van dt1_b
      result <- hits[, .(N = sum(n_point)), by = .rowid__]
  ...
  if (nrow(result_valid) == 0L || ncol(result_valid) == 0L) {
    result_full <- copy(dt1_out)[, N := 0L]    
  } else {
    result_full <- merge(
      dt1_out,
      result_valid,
